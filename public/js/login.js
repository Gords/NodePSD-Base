document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('htmx:afterSwap', function (event) {
    const registerResponseContent = document.querySelector(
      '#register-content #register-response'
    )
    if (
      registerResponseContent &&
      registerResponseContent.contains(event.detail.elt)
    ) {
  ...
  document.body.addEventListener('htmx:afterRequest', function (event) {
    if (event.detail.target.id === 'login-form') {
      ...
      const loginResponseContent = document.querySelector(
        '#login-content #login-response'
      )
      if (response.success) {
