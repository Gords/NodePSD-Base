// Behaviour for the file upload section in the user panel

// Function to initialize the file upload functionality
function initializeFileUpload(fileDropzoneComponent) {
	const fileList = []; // Store files for the currently active tab
  
	const allowedTypes = ["image/jpeg", "image/png", "application/pdf"];
  
	function handleFiles(files) {
	  for (let i = 0; i < files.length; i++) {
		const file = files[i];
		const isFileTypeAllowed = allowedTypes.includes(file.type);
		const isFileAlreadyAdded = fileList.some(
		  (existingFile) => existingFile.name === file.name
		);
  
		if (isFileTypeAllowed && !isFileAlreadyAdded) {
		  fileList.push(file);
		}
	  }
  
	  updateUploadFileList();
	}
  
	// Update the list of files in the current tab
	function updateUploadFileList() {
	  const fileListElement = fileDropzoneComponent.querySelector("#file-upload-list");
	  if (!fileListElement) return; // Guard against errors if element is not found
	  fileListElement.innerHTML = ""; // Clear existing entries
  
	  fileList.forEach((file, index) => {
      // Default document icon
      let fileIcon = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
</svg>
`;
      // Image icon
      if (file.type.includes("image")) {
        fileIcon = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Zm10.5-11.25h.008v.008h-.008V8.25Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
</svg>`;

        // PDF icon
      } else if (file.type === "application/pdf") {
        fileIcon = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
</svg>
`;
      }

      // X Icon to remove file from upload list
      const removeIcon = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>`;

// Create the list item
const listItem = document.createElement("div");
listItem.className = "file-item"; // Use a simple class for styling
listItem.innerHTML = `
  <span>${fileIcon} ${file.name}</span>
  <button class="remove-file">${removeIcon}</button> 
`;

// Attach click event listener to the remove button
listItem.querySelector(".remove-file").addEventListener("click", (event) => {
  event.stopPropagation();
  event.preventDefault();
  fileList.splice(index, 1);
  updateUploadFileList();
});

fileListElement.appendChild(listItem);
});
}

// Check if event listeners are already attached
const fileInput = fileDropzoneComponent.querySelector("#files-to-upload");
const dropArea = fileDropzoneComponent.querySelector("#drop-area");

// Only attach event listeners if they haven't been attached before
if (!fileInput.dataset.initialized) { 
fileInput.addEventListener("change", function () {
handleFiles(this.files);
});
fileInput.dataset.initialized = "true"; // Mark as initialized
}

if (!dropArea.dataset.initialized) {
dropArea.addEventListener("dragover", (e) => {
e.preventDefault();
gsap.to("#drop-area", {
  borderColor: "#ccc",
  background: "#3a4669",
  duration: 0.2,
});
});
dropArea.addEventListener("dragleave", (e) => {
e.preventDefault();
gsap.to("#drop-area", {
  borderColor: "#ccc",
  background: "#3a4669",
  duration: 0.2,
});
});
dropArea.addEventListener("drop", (e) => {
e.preventDefault();
const files = e.dataTransfer.files;
handleFiles(files);
gsap.to("#drop-area", {
  borderColor: "#ccc",
  background: "#283048",
  duration: 0.2,
});
});
dropArea.dataset.initialized = "true"; // Mark as initialized
}
}

// Attach the initializeFileUpload function to the htmx:afterSwap event
document.body.addEventListener('htmx:afterSwap', (event) => {
const fileDropzoneComponent = event.detail.target.querySelector('#file-dropzone-component');
if (fileDropzoneComponent) {
initializeFileUpload(fileDropzoneComponent);
}
});