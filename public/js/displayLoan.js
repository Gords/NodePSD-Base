document.addEventListener("htmx:afterSettle", () => {
	function calculateLoan() {
		...
		const monthlyPayment =
			...
			(1 - (1 + monthlyInterestRate) ** -installments);
		const totalInterest = monthlyPayment * installments - totalLoanAmount;
		const effectiveAnnualRate = (1 + monthlyInterestRate) ** 12 - 1;

		document.getElementById("loanOutput").textContent =
		...
		document.getElementById("amount2").textContent =
			monthlyPayment.toLocaleString("es-PY", {
				style: "currency",
				currency: "PYG",
			});
	}
	...
	document
		.getElementById("loan-calculator-component")
